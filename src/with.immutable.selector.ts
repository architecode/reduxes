...
export const createImmutableSelector: SelectorUtil.CreateSelector = (...keys: string[]) => ((_keys: string[]) => {
  return {
    ...
    set: (state, key, value) => Array.isArray(key) ? state.setIn([..._keys, ...key], value) : state.setIn([..._keys, key], value),
    push: (state, key, value) => Array.isArray(key) ? state.updateIn([..._keys, ...key], (list: any) => list.push(value)) : state.updateIn([..._keys, key], (list: any) => list.push(value)),
  } as Selector<any>;
})(keys);
